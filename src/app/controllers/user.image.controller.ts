...
const setImage = async (req: Request, res: Response): Promise<void> => {
    try{
        ...
        // Check user isn't trying to change another users image
        ...
        const imageData = req.body.binaryType;

        // Save image locally
        const imageFilename = `user_${userId}.${mime.extension(contentType)}`;
        const imagePath : string = `../../../storage/images`;

        fs.writeFileSync(imagePath, imageData);

        // Check if user already has a profile photo
        hasProfilePhoto = await checkUserHasProfileImage(userId);
